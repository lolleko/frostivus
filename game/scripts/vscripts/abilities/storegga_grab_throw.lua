...
function storegga_grab_throw:OnSpellStart()
    if IsServer() then
        ...
        self.hThrowBuff.flHeight =
            self.vSpawnLocation.z - GetGroundHeight(self:GetCaster():GetOrigin(), self:GetCaster())
        ...
        if self.hThrowTarget ~= nil then
            ...
            if self.hThrowTarget:IsRealHero() then
                ...
                if self.hThrowTarget:IsAlive() == false then
                    ...
                    self.hThrowTarget:AddNewModifier(
                        self:GetCaster(),
                        self,
                        "modifier_stunned",
