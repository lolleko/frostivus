...
function modifier_storegga_grab:OnIntervalThink()
    if IsServer() then
        ...
        local hBuff =
            self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_storegga_grabbed_buff", {})
        if hBuff ~= nil then
            self:GetCaster().flThrowTimer =
                GameRules:GetGameTime() + RandomFloat(self.min_hold_time, self.max_hold_time)
