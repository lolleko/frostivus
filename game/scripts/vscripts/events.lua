...
function GameMode:OnPlayerPickHero(data)
    ...
    GameRules:GetGameModeEntity():SetContextThink(
        DoUniqueString("playerINIT" .. plyID),
        function()
            if self:IsPVP() then
                ...
                PlayerResource:SpawnBuilding(
                    plyID,
                    "npc_frostivus_spirit_tree_pvp",
                    {origin = spawn:GetOrigin(), sizeX = 2, sizeY = 2, owner = hero},
                    function(args)
                ...
                local plyID = plyID
                PlayerResource:SetLumberCapacity(
                    plyID,
                    PlayerResource:GetLumberCapacity(plyID) + self:GetSpiritTree(plyID).LumberCapacity
                )
                PlayerResource:SetGoldCapacity(
                    plyID,
                    PlayerResource:GetGoldCapacity(plyID) + self:GetSpiritTree(plyID).GoldCapacity
                )
            end
        end,
        1
...
function GameMode:OnThink()
    if not self.nextItemCleanUp or self.nextItemCleanUp < GameRules:GetGameTime() then
        for _, itemContainer in pairs(Entities:FindAllByClassname("dota_item_drop")) do
            ...
            if item ~= nil then
                if
                    item:IsCastOnPickup() and
                        (item:GetAbilityName() == "item_mana_potion" or item:GetAbilityName() == "item_health_potion")
                 then
...
function GameMode:OnPlayerThink(plyID)
    ...
    if
        self:IsPVPHome() and PlayerResource:GetLastSaveTime(plyID) + 60 <= GameRules:GetGameTime() and
            GameRules:State_Get() == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS
     then
        PlayerResource:SetLastSaveTime(plyID, GameRules:GetGameTime())
        PlayerResource:StorePlayer(plyID)
    end
    if
        self:IsPVPHome() and PlayerResource:GetAutoHeroAIEnabled(plyID) and
            PlayerResource:GetLastAutoHeroAIThink(plyID) + 1 <= GameRules:GetGameTime()
     then
