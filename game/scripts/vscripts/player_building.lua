...
CustomGameEventManager:RegisterListener(
    ...
    function(...)
        PlayerResource:ProcessBuildingPreviewRequest(...)
    end
)
...
function CDOTA_PlayerResource:ProcessConstructionRequest(eventSourceIndex, data)
    ...
    if self:HasRequirements(plyID, building.Requirements, data.buildingName) then
        ...
        self:SpawnBuilding(
            plyID,
            data.buildingName,
            {origin = origin, sizeX = sizeX, sizeY = sizeY, rotation = data.rotation, randomAngles = randomAngles}
        )
    end
end
CustomGameEventManager:RegisterListener(
    "buildingRequestConstruction",
    function(...)
        PlayerResource:ProcessConstructionRequest(...)
    end
)
...
function CDOTA_PlayerResource:OnBuildingCheckArea(eventSourceIndex, data)
    ...
    local areaBlocked =
        not GridNav:IsPositionInSquare(GM:GetBuildingCenter(data.PlayerID), data.range, origin, sizeX, sizeY) or
        GridNav:IsAreaBlocked(origin, sizeX, sizeY)
    local previewModel = self:GetPreviewModel(plyID)
...
CustomGameEventManager:RegisterListener(
    "buildingCheckArea",
    function(...)
        PlayerResource:OnBuildingCheckArea(...)
    end
)
...
function CDOTA_PlayerResource:OnBuildingCheckSquare(eventSourceIndex, data)
    ...
    CustomGameEventManager:Send_ServerToPlayer(
        PlayerResource:GetPlayer(data.PlayerID),
        "buildingUpdateSquare",
        {blocked = areaBlocked, squareID = squareID}
    )
end
CustomGameEventManager:RegisterListener(
    "buildingCheckSquare",
    function(...)
        PlayerResource:OnBuildingCheckSquare(...)
    end
)
...
CustomGameEventManager:RegisterListener(
    "buildingCheckBlockedSquares",
    function(...)
        PlayerResource:OnBuildingCheckBlockedSquares(...)
    end
)
