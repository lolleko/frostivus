...
function OnStartTouch(data)
  if thisEntity.foodCompleted then return end
...
function SpawnFood(modelName, sequence)
  ...
  local propDyn = SpawnEntityFromTableSynchronous("prop_dynamic", {model = modelName, origin = spawnPoint:GetOrigin(), angles = Vector(0, RandomInt(0, 360), 0), DefaultAnim = sequence})
  ...
  if thisEntity.foodCount == thisEntity.requiredFood then
    ...
    for _, food in pairs(thisEntity.foodList) do
      local particle = ParticleManager:CreateParticle("particles/building/heal_ray.vpcf", PATTACH_POINT_FOLLOW, food)
      ParticleManager:SetParticleControlEnt(particle, 0, food, PATTACH_POINT_FOLLOW, "attach_hitloc", food:GetOrigin(), true)
      ParticleManager:SetParticleControlEnt(particle, 1, skull, PATTACH_POINT_FOLLOW, "attach_hitloc", skull:GetOrigin(), true)
...
function RessurrectThink()
  ...
  if not GameRules:IsGamePaused() then
    ...
    if thisEntity.resurrectStartTime + thisEntity.resurrectDuration <= GameRules:GetGameTime() then
      ...
      CreateUnitByNameAsync("npc_frostivus_boss_roshan", GetGroundPosition(skull:GetOrigin(), skull), true, nil, nil, DOTA_TEAM_BADGUYS, function(unit)
