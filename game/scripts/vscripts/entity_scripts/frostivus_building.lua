    ...
    if thisEntity.Building.DynamicModels then
        for _, dynMdl in pairs(thisEntity.Building.DynamicModels) do
            ...
            local propDyn =
                ...
                "prop_dynamic",
                {model = dynMdl.Model, origin = origin, angles = tovector(dynMdl.Angles), DefaultAnim = dynMdl.Sequence}
            )
            if color then
...
function WallRenderThink()
    ...
    for _, wall in pairs(adjacentWalls) do
        ...
        if slot and pos and not thisEntity.connectors[slot] then
            ...
            local ent =
                SpawnEntityFromTableSynchronous("prop_dynamic", {model = thisEntity:GetModelName(), origin = pos})
    ...
    for _, unitData in pairs(thisEntity.SpawnerUnits) do
        if
            GameRules:State_Get() ~= DOTA_GAMERULES_STATE_GAME_IN_PROGRESS or
                (unitData.Stage and unitData.Stage > GM:GetStage()) or
                (unitData.RequiredDifficulty and unitData.RequiredDifficulty > GM:GetDifficultyScalar())
         then
            -- keep spawnDelay until stage is ready
            local delay = unitData.InitialDelay or unitData.Interval
            if delay == 0 then
                delay = unitData.Interval
            end
        ...
        elseif unitData.NextSpawnTime <= gameTime then
            ...
            if not unitData.MaxAlive or #unitData.SpawnedUnits < unitData.MaxAlive then
                ...
                CreateUnitByNameAsync(
                    ...
                    thisEntity:GetOwner(),
                    thisEntity:GetTeam(),
                    function(unit) --thisEntity:GetTeam(), function(unit)
