...
function OrderThink()
    ...
    if thisEntity:IsRangedAttacker() then
        local buildings =
            Entities:FindBuildingInRadius(
            thisEntity:GetOrigin(),
            DOTA_TEAM_GOODGUYS,
            thisEntity:GetAttackRange(),
            FROSTIVUS_BUILDING_LOOKOUT
        )
        if #buildings > 0 then
        ...
        for k, bld in pairs(buildings) do
            ...
            if freeSquare then
                ...
                if
                    pathLength ~= -1 and (minDist == -1 or pathLength < minDist) and
                        pathLength <= thisEntity:GetCurrentVisionRange()
                 then
                    closest = bld
                    minDist = pathLength
                end
